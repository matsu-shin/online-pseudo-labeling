...
from pseudo_label_method import PseudoLabelThreshould
...
def main(cfg: DictConfig) -> None:
    ...
    pseudo_threshould = PseudoLabelThreshould(label_proportion=label_proportion,
    ...
    for epoch in tqdm(range(cfg.num_epochs)):
        # update pseudo label
        pseudo_label = pseudo_threshould.d
        ...
        train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=cfg.batch_size,
            shuffle=False,  num_workers=cfg.num_workers)
        pseudo_threshould.update_theta(model=model,
                                       loader=train_loader,
                                       device=cfg.device)

        # select the next k-set d_(t+1)
        pseudo_threshould.update_d()
    ...
    np.save('%s/result/loss_threshould_%s_%s_%s_%s_%s' %
    ...
    np.save('%s/result/acc_threshould_%s_%s_%s_%s_%s' %
            (cwd, cfg.dataset.name,
             cfg.dataset.negative_label,
             cfg.dataset.positive_label,
             cfg.dataset.num_bags,
